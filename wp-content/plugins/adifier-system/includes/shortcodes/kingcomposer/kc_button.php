...
if( $link['url'] == '#submit' ){
	if( is_user_logged_in() ){
		$link['url'] =  esc_url( add_query_arg( 'screen', 'new', get_author_posts_url( get_current_user_id() ) ));	
	}
	else{
		$link['url'] = '#login';
		$ex_class = 'af-modal';
...
else if( is_user_logged_in() && in_array( $link['url'], array( '#login', '#register' ) )){
	$link['url'] = esc_url( get_author_posts_url( get_current_user_id() ));
	$ex_class = '';
}
else if( !is_user_logged_in() && in_array( $link['url'], array( '#login', '#register' ) )){
	$ex_class = !empty( $ex_class ) ? $ex_class.' af-modal' : 'af-modal';
}
