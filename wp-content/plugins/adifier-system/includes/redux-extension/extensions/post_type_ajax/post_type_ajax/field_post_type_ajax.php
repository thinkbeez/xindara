...
if( !class_exists( 'ReduxFramework_post_type_ajax' ) ) {
    ...
    class ReduxFramework_post_type_ajax extends ReduxFramework {
        ...
        public function render() {
            ...
            $selected = array();
            $this->value = !empty( $this->value ) && !is_array( $this->value ) ? explode( ',', $this->value ) : array();
            if( !empty( $this->value ) ){
            ...
            echo '<input data-additional="'.htmlspecialchars( json_encode( $this->field['additional'] ) ).'" data-post_type="'.esc_attr( $this->field['post_type'] ).'" data-multiple="'.$multi.'" data-sortable="'.$sortable.'" type="hidden" value="'.esc_attr( join( ',', $this->value ) ).'" data-placeholder="' . $placeholder . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" class="redux-select-item post-type-ajax' . $this->field['class'] . '"' . $width . ' rows="6" data-selected="'.esc_attr( $selected ).'"/>';
        }      
